...
do
  ...
  API:CreateCmd("godmode", "Turns on all settings that prevent you from harm", function(args)
	...
	API:CreateCmd("AutoAk", "Loop gets a ak gun", function(args)
		local value = ChangeState(args[2],"AutoAk")
	end,nil,"[ON/OFF]")
	API:CreateCmd("AutoM9", "Loop gets a m9 gun", function(args)
		local value = ChangeState(args[2],"AutoM9")
	end,nil,"[ON/OFF]")
	API:CreateCmd("AutoM4A1", "Loop gets a m4a1 gun", function(args)
	...
	API:CreateCmd("Vereus", "FE Vereus", function(args)
		...
		API:Notif("Press Q = The reverse penance stare")
		wait(3)
		API:Notif("Press E = Doom Pillars")
		wait(3)
	        API:Notif("Press T = Unleashed evil ball")
		wait(3)
		API:Notif("Press Y = The hunt is on")
		wait(3)
		API:Notif("Press Z = CRAZY XESTER SWITCH!!!")
		wait(3)
		API:Notif("Press X = Re_*101011Dact/^ed.exe")
	end)
